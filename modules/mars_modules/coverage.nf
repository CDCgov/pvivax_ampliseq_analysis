...
process WT_cov {
   label 'nfNest'
    ...
    script:
        ...
        """
        ${params.pyscripts}/mars_scripts/Wt_cov.py -R ${ref} -G ${gff} -N '${sample_id}_depth.txt' -V ${voi} > ${sample_id}_coverage.csv
...
process Trim_Stats {
   label 'nfNest'
      ...
      script:
      """
      ${params.pyscripts}/mars_scripts/Reads_stats.py  -S "${sample_id}.stats.txt" -N "${sample_id}_coverage.txt" > "${sample_id}_readcoverage.csv"
