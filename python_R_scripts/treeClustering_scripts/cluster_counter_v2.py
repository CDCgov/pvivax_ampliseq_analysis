...
# # print(test1)
# test1 = test1[test1["Cluster0"].str.contains("Read") == False]
...
# Read the augmented clstr file
with open(args.x.name) as file:
...
for line in lines:
    if line.startswith(">Cluster"):
        ...
        # Count the number of sequences in the cluster
        cluster_sizes[cluster_id] += 1
# print(cluster_sizes)
# # Write cluster sizes to the summary file
with open(args.y.name, 'w') as summary_file:
    for cluster_id, size in cluster_sizes.items():
        cluster_id = cluster_id.replace(">","")
        summary_file.write(f"{cluster_id},{size}\n")