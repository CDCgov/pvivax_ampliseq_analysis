...
parser.add_argument("-i", "--hapSheet")
parser.add_argument("-c", "--inChromosome")
...
inSheet = pd.read_csv(args.hapSheet, sep = "\t", keep_default_na=False).set_index("Seq_ID")
# print(inSheet.head())

selectedDF = inSheet.filter(regex = args.inChromosome)
print(selectedDF.head())

# args.inChromosome
# paste0("./", this_chrom,"/",todays_date,"_",this_chrom,"_haplotype_data_sheet.txt")
...
fileNameLocal =  today + args.inChromosome +"_haplotype_data_sheet.txt"
selectedDF.to_csv(fileNameLocal, index = True, header = True, sep = "\t", index_label = "Seq_ID")
...
# Seq_ID_full = [s for s in Seq_ID if os.stat(path.join(tempPath,s)).st_size != 0 ] #get list of files that are not empty
