...
kable_out <- knitr::kable(merge2, "html", padding = 40, line_sep = 2) %>% kableExtra::kable_styling(bootstrap_options = c("striped", "hover"), full_width = F, position = "center", font_size = 12)
...
# merge2 <- merge2[,-9]
# write.csv(merge2, file = "tryEmbed.csv", quote = F, row.names =F)

# #make the rmarkdown file
# rmarkdown::render("Pvivax_markdown.Rmd", "html_document",  run_pandoc = TRUE)
...
file.copy(from = "Pvivax_markdown.html", to = htmlName)
